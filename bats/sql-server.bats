...
@test "test manual commit" {
    ...
    # Repeat action with custom author
    multi_query 0 "
    SET @@repo1_head=commit('-m', 'test commit message', '--author', 'John Doe <john@example.com>');
    INSERT INTO dolt_branches (name,hash) VALUES ('test_branch', @@repo1_head);"

    # validate new branch was created and author parameters are correct
    server_query 0 "SELECT latest_committer,latest_committer_email FROM dolt_branches LIMIT 1" "latest_committer,latest_committer_email\nJohn Doe,john@example.com"
